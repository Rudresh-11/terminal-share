      ...
      <Card className="w-full max-w-md border-border/50 bg-background shadow-2xl">
        ...
        <CardContent className="space-y-6 p-6">
          ...
          {currentProvider === "local" && (
            <div className="rounded-sm border border-border/50 bg-muted/30 p-3">
              <p className="text-[10px] leading-relaxed text-muted-foreground">
                Run Ollama locally, expose it with{" "}
                <code className="font-mono text-foreground">cloudflared tunnel --url http://localhost:11434</code>,
                and paste the resulting <code className="font-mono text-foreground">trycloudflare.com</code> URL
                above. Ollama must allow this site&apos;s origin (set{" "}
                <code className="font-mono text-foreground">OLLAMA_ORIGINS=*</code> or your app&apos;s origin) or the
                browser will block the request as CORS.
              </p>
            </div>
          )}
        </CardContent>
